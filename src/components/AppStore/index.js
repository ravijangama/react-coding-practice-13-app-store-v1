...
class AppStore extends Component {
  state = {isTabId: tabsList[0].tabId, searchInput: ''}

  clickTabItem = id => {
    this.setState({isTabId: id})
  }

  getFilteredApps = () => {
    const {isTabId} = this.state
    const filteredList = appsList.filter(
      eachApp => eachApp.category === isTabId,
    )
    return filteredList
  }

  getSearchResults = filteredAppList => {
    const {searchInput} = this.state
    const results = filteredAppList.filter(eachApp =>
  ...
  render() {
    const {searchInput, isTabId} = this.state
    const filteredAppList = this.getFilteredApps()
    const searchResults = this.getSearchResults(filteredAppList)
    const jsxElement = (
          ...
          <ul className="tab-ul-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === isTabId}
              />
            ))}
          </ul>
